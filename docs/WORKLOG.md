...
## 2026-08-09 — Architecture diagnosis and primary sweep

*(Entry reconstructed 2026-08-24 after commit `fd1f10f` deleted this file; it had
never been committed. Faithful to the original — verify before citing.)*

**What was found.** SB3's `MultiInputPolicy` builds a `CombinedExtractor`, which
gives a `Dict` entry a CNN only if it is an *image space*. A `(3, 50, 8)` price
tensor is not, so it took the `nn.Flatten()` branch: 1200 unrelated inputs
concatenated with `w_prev` into a 1209-dim vector. No convolution existed anywhere
in the trained models, and everything Jiang Eq. 18 encodes was destroyed before
the network saw it.

**What was built**

- `experiments/extractors.py` — `AssetSharedConvExtractor`, two convolutions with
  `(k, 1)` kernels so the same filter applies to every asset column (Jiang's IIE
  property). 1209 features -> 169; 77,376 first-layer params -> 1,960 encoder
  params. Deliberately **not** full EIIE: changing only the encoder keeps the
  causal claim attributable. `EIIEExtractor` kept as an alias (probe checkpoints
  were pickled under that name).
- `train.py` — `--extractor {flatten,conv}`, `--action-bound`, `--models-dir`, and
  a freeze guard (`--freeze-patience`, default 8).
- `portfolio_env.py` — `ACTION_BOUND` 10 -> 5, now a per-env parameter so the
  ablation environment stays reproducible. All 7 unit tests still pass.
- `policy_diagnostic.py` — verdict logic corrected. The old rule reported a policy
  as healthy whenever its weights moved, even with a decorrelated probe of exactly
  zero. Four verdicts now: FROZEN / MARKET-BLIND / negligible / responds.

**Primary sweep** (conv encoder, 500k, bound +/-5): PPO 5 seeds, A2C 5 seeds,
DDPG stopped at 2.

Test-window fAPV: PPO **1.2074 +/- 0.0893** - A2C 1.1982 +/- 0.0341 -
DDPG 1.1720 (n=2) - Buy & Hold 1.1506 - UCRP 1.1532.

All three algorithms now beat the passive benchmarks on **return** (ablation: none
did). None beats them on **Sharpe** or drawdown, expected given the objective has
no risk term.

Diagnostic on test: **4 of 12 policies respond to the price tensor, all PPO.**
A2C is market-blind on all 5 seeds despite the identical encoder. DDPG frozen on
both.

**DDPG stopped deliberately.** Seed 0 froze after 295 minutes; seed 1 froze at the
identical validation value (1.2390). With four independent demonstrations of the
same saturation failure across two bounds and two extractors, the remaining ~16 h
would have produced three more frozen policies. Reported as n=2 throughout.

**Corrections made during this work (recorded so they are not repeated)**

- +/-3 was proposed for the action bound before checking; the working PPO policy
  uses a spread of 6.90, which +/-3 cannot express. +/-5 is the measured choice.
- Narrowing the bound was expected to fix DDPG. It did not: 100% of outputs are
  railed at +/-5 exactly as at +/-10. The bound was the blast radius, not the cause.
- A bound mismatch was believed to distort evaluation of old checkpoints. It does
  not; SB3 clips to the model's own stored action space.
- The hypothesis that Liang's PG wins because it carries Jiang's architecture
  predicts A2C+conv should work. It did not. Architecture alone does not explain
  their ranking.

**Next:** supervisor decision on framing (comparison vs diagnostic study), DDPG
n=2 vs completing seeds 2-4, and the yfinance/Microsoft data source question.

---

## 2026-08-24 — Colab results notebook; A2C batch-size gap identified

**Probed the checkpoints directly** (5 decorrelated test regimes). Confirmed and
refined the §9.2 account:

| model | action range | % dims railed | spread across regimes |
|---|---|---|---|
| DDPG conv s0 | [-5.000, +5.000] | 100% | 0.000000 |
| DDPG flat s0 | [-10.000, +10.000] | 100% | 0.000000 |
| A2C conv s2 | **[-3.752, +2.150]** | **0%** | 0.000000 |
| PPO conv s2 | [-1.986, +1.955] | 0% | 2.853 |
| PPO conv s3 | [-0.733, +1.345] | 0% | 0.000000 |

**A2C is NOT saturated** — outputs sit well inside the bound yet are identical
across regimes. So DDPG's saturation account does not transfer to A2C, and
narrowing the bound cannot help it. Two distinct failure modes, not one.
(SB3's DDPG actor is tanh-squashed, `td3/policies.py:58` `squash_output=True`;
PPO/A2C use unsquashed Gaussians, so they have no saturating nonlinearity to kill.)

**Why A2C collapses.** The reward `log(mu_t * (y_t . w_t))` decomposes into
`log mu_t` (transaction cost: deterministic, noise-free, negative when trading)
plus `log(y_t . w_t)` (market return: SNR ~0.035). The only reliably learnable
signal is "moving costs money". A2C takes one unclipped step per **16 samples**
at 2.3x PPO's learning rate with the max-variance advantage estimator
(gae_lambda=1.0), so the market term averages to noise while the cost term does
not. PPO averages over **2048** samples (~11x better SNR per update) and clips
each step.

**`n_steps=16` is a project choice, not an SB3 default** (SB3's A2C default is 5,
`a2c/a2c.py:71`; PPO's 2048 is the SB3 default). No experiment justifies 16. This
is a **128x batch gap** and it is the fair challenge to expect from the supervisor.
Next experiment: A2C with `--n-steps 2048 --lr 3e-4`, which is essentially PPO
without the clip and isolates one variable.

**Built `notebooks/colab_results.ipynb`** — 27 cells, evaluates all three
algorithms and renders every table and figure inline (nothing read from private
storage). Verified end to end locally: evaluation reproduced the committed
`results/evaluation_conv/` byte-identically. Fixed one wrong flag
(`plot_training.py` takes `--out-dir`, not `--out`).

**File-loss note.** `docs/WORKLOG.md` was deleted by `fd1f10f`, restored, and
deleted again the same day while untracked. `requirements.txt` was lost twice by
the same mechanism. **Commit recovered files immediately** — untracked files do
not survive `git clean`.
